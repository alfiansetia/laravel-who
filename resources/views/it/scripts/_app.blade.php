...
<script>
    const API_URL = '{{ route('api.it.index') }}';
    const DETAIL_URL = '{{ route('api.it.index') }}';
    const PRINT_URL = '{{ url('it') }}';
    let currentPage = 1;
    ...
    function loadData() {
        ...
        $.ajax({
            ...
            error: function(xhr) {
                const msg = xhr.responseJSON?.message || 'Gagal memuat data IT!';
                $tbody.html(
                    `<tr><td colspan="8" class="text-center text-danger py-4"><i class="fas fa-exclamation-triangle mr-2"></i>${escapeHtml(msg)}</td></tr>`
                );
            }
        });
    }
    ...
    function renderTable(data) {
        ...
        data.forEach((row) => {
            ...
            const destination = Array.isArray(row.location_dest_id) ? escapeHtml(row.location_dest_id[1]) : '-';
            const origin = escapeHtml(row.origin || '-');
            const state = escapeHtml(row.state || '-');
            const noteWh = row.note_to_wh ? (row.note_to_wh.length > 40 ? escapeHtml(row.note_to_wh.substring(0,
                40)) + '...' : escapeHtml(row.note_to_wh)) : '-';
            const noteItr = row.note_itr ? (row.note_itr.length > 40 ? escapeHtml(row.note_itr.substring(0,
                40)) + '...' : escapeHtml(row.note_itr)) : '-';
            ...
            html += `<tr data-id="${row.id}">
                ...
                <td class="text-center">
                    <button type="button" class="btn btn-action btn-success btn-print-it" data-id="${row.id}" title="Print IT">
        ...
        // Action button: Print IT
        $('#tableBody').on('click', '.btn-print-it', function(e) {
            e.preventDefault();
            e.stopPropagation();
            const itId = $(this).data('id');
            window.open(`${PRINT_URL}/${itId}/print`, '_blank');
        });
    }
    ...
    function renderPagination(page, totalPages, total) {
        ...
        $pag.off('click', '.page-btn').on('click', '.page-btn', function() {
            ...
            loadData();
            $('html, body').animate({
                scrollTop: $('#tableIT').offset().top - 80
            }, 200);
        });
    }
    ...
    function loadDetail(id) {
        ...
        $.ajax({
            ...
            success: function(res) {
                ...
                // Modal title & info
                $('#modal_productLabel').html(`<i class="fas fa-list mr-2"></i>Item IT: ${d.name || id}`);
                $('#modal_status').html(
            ...
            error: function(xhr) {
                const msg = xhr.responseJSON?.message || 'Gagal memuat detail IT!';
